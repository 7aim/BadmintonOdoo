...
class SportMembership(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', 'Yeni') == 'Yeni':
            vals['name'] = self.env['ir.sequence'].next_by_code('sport.membership') or 'SM001'
        return super(SportMembership, self).create(vals)


class SportAttendance(models.Model):
    _name = 'sport.attendance'
    _description = 'Dərsə İştirak'
    
    membership_id = fields.Many2one('sport.membership', string="Üzvlük", required=True)
    schedule_id = fields.Many2one('sport.schedule', string="Qrafik", required=True)
