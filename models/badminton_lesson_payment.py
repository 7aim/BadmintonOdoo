...
class BadmintonLessonPayment(models.Model):
    _name = 'badminton.lesson.payment'
    _description = 'Badminton Dərs Ödənişi'
    _order = 'real_date desc'
    
    lesson_id = fields.Many2one('badminton.lesson.simple', string="Dərs Abunəliyi", required=True, ondelete='cascade')
    ...
    def default_get(self, fields_list):
        ...
        if self._context.get('default_lesson_id'):
            lesson = self.env['badminton.lesson.simple'].browse(self._context.get('default_lesson_id'))
    ...
    def create(self, vals):
        ...
        lesson = self.env['badminton.lesson.simple'].browse(vals['lesson_id']) if vals.get('lesson_id') else False
        if lesson:
    ...
    def action_print_receipt(self):
        ...
        return {
            ...
            'report_name': 'volan_yasamal.report_badminton_payment_receipt_document',
