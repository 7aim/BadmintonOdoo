...
class BadmintonProductSale(models.Model):
    ...
    # Satış xətləri
    sale_line_ids = fields.One2many('badminton.product.sale.line', 'sale_id', string="Raketka")
    ...
    def action_confirm(self):
        ...
        for sale in self:
            ...
            for line in sale.sale_line_ids:
                ...
                # Stok hərəkəti yarat
                self.env['badminton.stock.movement'].create({
...
class BadmintonProductSaleLine(models.Model):
    _name = 'badminton.product.sale.line'
    _description = 'Badminton Raketka Satış Xətti'
    
    sale_id = fields.Many2one('badminton.product.sale', string="Satış", required=True, ondelete='cascade')
    product_id = fields.Many2one('badminton.product', string="Raketka", required=True)
