...
class BasketballLessonFreezeWizard(models.TransientModel):
    _name = 'basketball.lesson.freeze.wizard'
    _description = 'Basketbol Abunəlik Dondurma Sehrbazı'
    
    lesson_id = fields.Many2one('basketball.lesson.simple', string="Abunəlik", required=True, ondelete='cascade')
    ...
    def action_confirm_freeze(self):
        ...
        freeze_record = self.env['basketball.lesson.freeze'].create({
            'lesson_id': self.lesson_id.id,
            'freeze_start_date': self.freeze_start_date,
            'freeze_end_date': self.freeze_end_date,
            'notes': self.notes or "Basketbol abunəlik donduruldu",
        })
        ...
        return {
            ...
            'params': {
                ...
                'message': f'Basketbol abunəliyi {self.freeze_days} gün müddətinə donduruldu.',
                'type': 'success',
                'sticky': False,
            }
        }
