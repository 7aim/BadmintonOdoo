...
class BasketballLessonFreeze(models.Model):
    _name = 'basketball.lesson.freeze'
    _description = 'Basketbol Abunəlik Dondurması'
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('basketball.lesson.freeze') or 'BBFRZ/001'
            
        return super(BasketballLessonFreeze, self).create(vals)
