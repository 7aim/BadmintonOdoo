...
class BadmintonLessonFreeze(models.Model):
    ...
    lesson_id = fields.Many2one('badminton.lesson.simple', string="Abunəlik", required=True, ondelete='cascade')
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('badminton.lesson.freeze') or 'FRZ/001'
