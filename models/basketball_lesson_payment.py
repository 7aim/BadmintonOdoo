...
class BasketballLessonPayment(models.Model):
    ...
    _description = 'Basketbol Dərs Ödənişi'
    _order = 'payment_date desc'
    
    lesson_id = fields.Many2one('basketball.lesson.simple', string="Dərs Abunəliyi", required=True, ondelete='cascade')
    ...
    # Kassa əməliyyatı
    cash_flow_id = fields.Many2one('volan.cash.flow', string="Kassa Əməliyyatı", readonly=True)
    ...
    def default_get(self, fields_list):
        """Default məbləğ olaraq lesson_fee götür"""
        res = super(BasketballLessonPayment, self).default_get(fields_list)
        
        # Əgər context-də lesson_id varsa
        if self._context.get('default_lesson_id'):
            lesson = self.env['basketball.lesson.simple'].browse(self._context.get('default_lesson_id'))
    ...
    def create(self, vals):
        ...
        if payment.lesson_id and payment.amount > 0:
            ...
            cash_flow = self.env['volan.cash.flow'].create({
                'name': f"Basketbol dərs ödənişi: {payment.lesson_id.name} - {month_names.get(payment.payment_month, '')}",
                'date': cash_date,
                'amount': payment.amount,
                'transaction_type': 'income',
                'category': 'basketball_lesson',
                'partner_id': payment.partner_id.id,
                'related_model': 'basketball.lesson.payment',
    ...
    def write(self, vals):
        ...
        res = super(BasketballLessonPayment, self).write(vals)
    ...
    def unlink(self):
        ...
        return super(BasketballLessonPayment, self).unlink()
