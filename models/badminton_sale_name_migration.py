...
class BadmintonSaleNameMigration(models.TransientModel):
    _name = 'badminton.sale.name.migration'
    ...
    def default_get(self, fields_list):
        ...
        sales = self.env['badminton.sale'].search([])
        duplicate_count = 0
        seen_names = set()
        for sale in sales:
    ...
    def action_migrate_sale_names(self):
        ...
        BadmintonSale = self.env['badminton.sale']
        ...
        self.env.cr.execute("""
            ALTER TABLE badminton_sale DROP CONSTRAINT IF EXISTS badminton_sale_name_unique;
        """)
        ...
        for sale in sales:
            try:
                ...
                new_name = self.env['ir.sequence'].next_by_code('badminton.sale')
                if not new_name:
                    new_name = f"S-{sale.id:05d}"
                
                # SQL ilə birbaşa yenilə (ORM-i bypass et)
                self.env.cr.execute("""
                    UPDATE badminton_sale 
                    SET name = %s 
                    WHERE id = %s
                """, (new_name, sale.id))
        ...
        self.env.cr.execute("""
            ALTER TABLE badminton_sale 
            ADD CONSTRAINT badminton_sale_name_unique UNIQUE(name);
        """)
